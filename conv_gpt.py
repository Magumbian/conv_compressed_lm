...
Run examples:
  Single GPU:
    python conv_gpt.py --max_steps 2000 --batch_size 16

  Multi-GPU (Runpod / 8xH100):
    torchrun --standalone --nproc_per_node=8 conv_gpt.py --max_steps 8000 --batch_size 8 --grad_accum 8
"""
    ...
    if world_size > 1:
        ...
        if backend == "nccl":
            if not torch.cuda.is_available():
                raise RuntimeError("WORLD_SIZE > 1 with backend=nccl requires CUDA")
            if local_rank >= torch.cuda.device_count():
                raise RuntimeError(
                    f"Invalid local_rank={local_rank} for visible CUDA devices={torch.cuda.device_count()}"
                )
            # Avoid NCCL cuMem allocator path issues seen on some remote H100 stacks.
            os.environ["NCCL_CUMEM_ENABLE"] = "0"
            os.environ.setdefault("TORCH_NCCL_ASYNC_ERROR_HANDLING", "1")
            torch.cuda.set_device(local_rank)
            # Match baseline behavior: bind NCCL PG init to the local CUDA device.
            # Allow explicit opt-out via DIST_INIT_DEVICE_BIND=0.
            use_device_bind = os.environ.get("DIST_INIT_DEVICE_BIND", "1") == "1"
            if use_device_bind:
                try:
...
class EmbeddingSemanticConv(nn.Module):
    ...
    def __init__(self, d_model: int, d_attn: int, kernel_size: int, dilation: int):
        super().__init__()
        # Causal conv: only look at current/past tokens (no right-context leakage).
...
class ConvBlock(nn.Module):
    """
    Single-path non-residual block:
    conv-compress embeddings -> low-parameter attention -> expand -> MLP.
    """
...
class ParameterGolfLM(nn.Module):
    """
    Parameter-efficient causal LM using single-node blocks.
    """
        ...
        self.blocks = nn.ModuleList(
            [
                ConvBlock(
                    ...
                    conv_dilation=cfg.conv_dilation,
                    dropout=cfg.dropout,
                )
...
def build_arg_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Parameter Golf single-kernel direct-training script")
    ...
    p.add_argument("--conv_dilation", type=int, default=2)
    ...
    p.add_argument(
        ...
        help="Micro-steps cap (before grad_accum reduction). Keep large to let --time_limit_minutes control stopping.",
    )
    ...
    p.add_argument(
        ...
        action="store_true",
        default=False,
        help="Use single-process torch.nn.DataParallel across all visible GPUs (no torchrun/NCCL).",
    )
    ...
    p.add_argument("--out_path", type=str, default="conv_gpt_ckpt.pt")
...
def main() -> None:
    ...
    if args.data_parallel and world_size > 1:
        raise ValueError("--data_parallel cannot be used with torchrun/WORLD_SIZE>1. Run with plain python.")
    # Keep model initialization identical across ranks; data sampling is already rank-partitioned.
    ...
    if args.compile and (not args.data_parallel) and hasattr(torch, "compile"):
    ...
    if world_size > 1:
        ...
        if n_gpus <= 1:
            raise ValueError(f"--data_parallel requires at least 2 visible GPUs, found {n_gpus}")
    ...
    # Baseline-style wallclock handling: synchronize stop across ranks and smoothly warm down LR
    # as we approach the cap.
    max_wallclock_ms = 1000.0 * (args.time_limit_minutes * 60.0) if args.time_limit_minutes > 0 else None
    warmdown_updates = 1200

    def lr_mul(update_idx: int, elapsed_ms: float) -> float:
        if warmdown_updates <= 0:
            return 1.0
        if max_wallclock_ms is None:
            warmdown_start = max(run_update_budget - warmdown_updates, 0)
            if warmdown_start <= update_idx < run_update_budget:
                return max((run_update_budget - update_idx) / max(warmdown_updates, 1), 0.0)
            return 1.0
        step_ms = elapsed_ms / max(update_idx, 1)
        warmdown_ms = warmdown_updates * step_ms
    ...
    while True:
        last_step = global_step >= target_global_step or (stop_after_step is not None and global_step >= stop_after_step)
        if last_step:
            if stop_after_step is not None and global_step < target_global_step and is_main_process():
                elapsed_ms = 1000.0 * (time.perf_counter() - t0)
                print(
                    f"stopping_early: wallclock_cap train_time={elapsed_ms:.0f}ms "
                    f"micro_step={global_step} update_step={update_step}"
                )
            break
        ...
        if is_dist():
            micro_step_idx = global_step % args.grad_accum
            model.require_backward_grad_sync = micro_step_idx == (args.grad_accum - 1)  # type: ignore[attr-defined]
        ...
        if global_step % args.grad_accum == 0:
            elapsed_ms = 1000.0 * (time.perf_counter() - t0)
            base_lr = cosine_lr(run_update_step, run_update_budget, args.warmup_steps, args.lr, args.min_lr)
            lr = base_lr * lr_mul(run_update_step, elapsed_ms)
            set_lr(optimizer, lr)
            ...
            if should_log:
                parallel_replicas = world_size
                if args.data_parallel and device.type == "cuda":
                    parallel_replicas = max(1, torch.cuda.device_count())
                tok_per_step = args.batch_size * args.seq_len * args.grad_accum * parallel_replicas
                if is_dist():
                ...
                if is_main_process():
                    approx_training_time_ms = 1000.0 * (time.perf_counter() - t0)
                    print(
                        ...
                        f"step_avg={approx_training_time_ms / max(update_step, 1):.2f}ms"
                    )
                log_loss_sum = 0.0
                log_updates = 0
            ...
            if is_dist() and max_wallclock_ms is not None:
                reached_cap_tensor = torch.tensor(int(reached_cap), device=device)
                dist.all_reduce(reached_cap_tensor, op=dist.ReduceOp.MAX)
                reached_cap = bool(reached_cap_tensor.item())
            if stop_after_step is None and reached_cap:
                stop_after_step = global_step

    if is_dist():
        model.require_backward_grad_sync = True  # type: ignore[attr-defined]
        dist.barrier()
